...
if selected_job:
    ...
    with st.form(form_key, clear_on_submit=True):
            ...
            if submitted:
                ...
                if errors:
                    st.error(f"❌ Please fill all required fields: {', '.join(errors)}")
                else:
                    with st.spinner("📤 Submitting your application..."):
                        try:
                            ...
                            # Professional Email using existing EmailService
                            try:
                                ...
                                <p style="font-size: 15px; line-height: 1.6;">Thank you for your interest in joining <strong>Churchgate Group</strong>. We are pleased to confirm that your application has been successfully received and is now under review.</p>
                                
                                <div class="info-box">
                                    <p style="margin: 0;"><strong>📋 Tracking ID:</strong> <span style="color: #CC0000; font-size: 18px; font-weight: bold;">{tracking_id}</span></p>
                                    <p style="margin: 10px 0 0 0;"><strong>💼 Position:</strong> {position_name}</p>
                                    <p style="margin: 5px 0 0 0;"><strong>🏢 Department:</strong> {job_details.get('department', 'N/A') if job_details else 'N/A'}</p>
                                    <p style="margin: 5px 0 0 0;"><strong>📍 Location:</strong> {job_details.get('location', 'N/A') if job_details else 'N/A'}</p>
                                    <p style="margin: 5px 0 0 0;"><strong>📅 Date:</strong> {datetime.now().strftime('%B %d, %Y')}</p>
                                    <p style="margin: 10px 0 0 0;"><strong>📊 Status:</strong> <span style="color: #D4AF37;">Under Review</span></p>
                                </div>
                                
                                <div class="info-box">
                                    <h3 style="margin: 0 0 10px 0; color: #1a1a1a;">📌 What Happens Next?</h3>
                                    <ul style="padding-left: 20px; margin: 0; color: #5c4a2a; line-height: 1.8;">
                                        <li>Our HR team will review your application within <strong>5-7 business days</strong></li>
                                        <li>Shortlisted candidates will be contacted for <strong>initial screening</strong></li>
                                        <li>You can check your application status anytime using your <strong>Tracking ID</strong></li>
                                ...
                                es.send_email(
                                    to_email=email,
                                    subject=f"Application Received - {position_name}",
                                    body=email_body
                                )
                            except Exception as ex:
        ...
        for job in filtered_jobs:
            ...
            with st.expander(f"{dept_icon} {clean_title} — {job['dept']} | {job['location']}", expanded=False):
                ...
                with c2:
                    ...
                    # WhatsApp with plain text (no emojis that break)
                    whatsapp_text = f"*We're Hiring!*\n\n*{clean_title}*\nChurchgate Group\n{location}\n{job_type}\n{department}\n\nApply here: {share_url}"
                    whatsapp_url = f"https://wa.me/?text={urllib.parse.quote(whatsapp_text)}"
                    
                    # Twitter/X
                    twitter_text = f"We're hiring! {clean_title} at Churchgate Group - {location}\n\nApply now: {share_url}\n\n#Hiring #Jobs #Nigeria #Careers"
                    twitter_url = f"https://twitter.com/intent/tweet?text={urllib.parse.quote(twitter_text)}"
                    
                    # Email
                    email_subject = f"Job Opportunity: {clean_title} at Churchgate Group"
                    email_body = f"Hi,%0D%0A%0D%0AI thought you might be interested in this opening:%0D%0A%0D%0A{clean_title}%0D%0A{location}%0D%0A{job_type}%0D%0A{department}%0D%0A%0D%0AApply: {share_url}"
